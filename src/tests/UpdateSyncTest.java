...
public class UpdateSyncTest {
	...
	public static void test1() {
		try {
			...
			URL[] bootstrap = TestUtils.makeURLs(0);


			System.out.println(Arrays.toString(bootstrap));
			ChordWrapper peer0 = new ChordWrapper(bootstrap[0], bootstrap[1], bootstrap[2],
					"owner/");

			URL[] urls = TestUtils.makeURLs(1);
			ChordWrapper peer1 = new ChordWrapper(urls[0], urls[1], urls[2],
				bootstrap[0], bootstrap[1], bootstrap[2], "peer1/");


			urls = TestUtils.makeURLs(2);
			ChordWrapper peer2 = new ChordWrapper(urls[0], urls[1], urls[2],
					bootstrap[0], bootstrap[1], bootstrap[2], "peer2/");

			System.out.println("created 3 peers");

			String s = "File version 1";
			peer0.fileManager.writeToRelativeFile("coolFile.txt", s.getBytes(), false);
	...
	public static void test2() {
		try {
			...
			ChordWrapper peer4 = new ChordWrapper(urls[0], urls[1], urls[2],
					bootsrap[0], bootsrap[1], bootsrap[1], "peer4/");
			...
			peer0.fileManager
					.writeToRelativeFile(filename, s1.getBytes(), false); // p0 -> v1
			...
			peer1.fileManager.writeToRelativeFile(filename, s2.getBytes(), false);
			...
			peer2.fileManager.writeToRelativeFile(filename, s3.getBytes(), false);
			...
			peer3.fileManager.writeToRelativeFile(filename, s4.getBytes(), false);
			...
			peer4.fileManager.writeToRelativeFile(filename, s5.getBytes(), false);
